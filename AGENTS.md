...
- `workflows/` — pluggable workflow packages: each workflow lives in its OWN package directory named after it, with the implementation in `workflow.py` (`build(ctx)` decorated with `@register("key")`); the package `__init__.py` is a thin re-export of `build` only — never put logic in `__init__.py`. Selected via env `QR_WORKFLOW` (default `simple`); the registry itself lives in `workflows/registry.py` (re-exported from `workflows/__init__.py`, which also imports every workflow package to trigger registration). Add new workflows by creating a package + importing it at the bottom of `workflows/__init__.py`.
  ...
  - `simple_agent/` — SimpleAgentWorkflow (chat assistant, tools rooted at home or `QR_SIMPLE_ROOT`). Deliberately a single-module package — don't split it.
  - `dev_review/` — DevProcessReviewWorkflow, split by concern: `prompts.py` (ALL copy: intent/review prompts, refuse texts, REVIEW_TYPES — wording changes go here only), `state.py` (`Intent` schema + `ReviewState`), `workflow.py` (LangGraph pipeline `classify_intent → resolve_project → init_workspace → {clone_code ∥ clone_knowbase ∥ fetch_doc} → review (deepagents) → write_result` + `refuse` node, `load_projects`, `_sync_repo`). Tests monkeypatch `create_deep_agent` on the `workflow` modules (`workflows.dev_review.workflow`), not on the packages. Delivery stays OUTSIDE the graph (adapter in `main.py`) by design — the graph must stay channel-agnostic for the future FastAPI entry. The workspace is PERSISTENT per project (`workspaces/<project>/`): `_sync_repo` clones once then pulls, requirement docs are cached as `docs/<title>-<revision>.md` (same revision reuses the local file), results go to timestamped `result/review-<ts>.md` files. The review prompt names the exact doc file + label (`《title》rN`) because `docs/` holds historical versions.
