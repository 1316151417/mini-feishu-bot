...
- `workflows/` — a NAMESPACE package: NO `__init__.py` anywhere under it (the user dislikes them; don't reintroduce). Each workflow lives in its own directory named after it, with the implementation in `workflow.py` (`build(ctx)` decorated with `@register("key")`). The registry lives in `workflows/registry.py` and imports every `workflows.<name>.workflow` at the BOTTOM of the file to trigger registration (bottom placement avoids the import cycle). Selected via env `QR_WORKFLOW` (default `simple`). Add a workflow by creating a directory + `workflow.py`, then adding the import at the bottom of `registry.py`.
